...
class DynamicToolLoader:
    ...
    def _load_single_tool(self, tool_info: Dict[str, Any]) -> Tool:
        ...
        doc = inspect.getdoc(main_func) or tool_info['description']
        
        # Create enhanced tool wrapper with proper parameter handling
        def wrapped_tool_function(*args, **kwargs):
            try:
                ...
                if 'args' in kwargs and 'kwargs' in kwargs:
                    # Convert from LLM format to function parameters
                    param_name = list(sig.parameters.keys())[0]
                    if kwargs['args']:
                        result = main_func(kwargs['args'])
                    else:
                        result = main_func()
                elif len(sig.parameters) == 1 and kwargs:
                    # Single parameter function
                    param_name = list(sig.parameters.keys())[0]
                    if param_name in kwargs:
                        result = main_func(kwargs[param_name])
                    else:
                        # Try to use the first value if parameter name doesn't match
                        result = main_func(list(kwargs.values())[0])
                else:
        ...
        wrapped_tool_function.__doc__ = doc
        
        # Create Tool instance with proper schema
        tool = Tool(
            name=tool_name,
            func=wrapped_tool_function,
            description=doc
        )
        print("💡💡\nTool Creation :",tool)
        return tool
